...
w:- pos(A, B), A1 is (A - 1), map(A1, B, _), myplant(A1,B,_,_,_,_,_,_), write('Oops, you can\'t step over the planted tile!\n\n'), map, !.
...
d:- pos(A, B), B1 is (B + 1), map(A, B1, _), myplant(A,B1,_,_,_,_,_,_), write('Oops, you can\'t step over the planted tile!\n\n'), map, !.
